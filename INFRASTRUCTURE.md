...
* **2026-05-01 (later)** — Section 1 (Google Apps Script) — hardening confirmed live in prod (operator manually deployed via "Manage Deployments → New Version"). Canonical source mirrored to `marketing/waitlist-script/Code.gs` + `README.md` so the repo holds the audit trail. Section 1 (and `INFRASTRUCTURE.md §2 Stripe Billing`) — `/api/billing/plan-change` wired to real Stripe API: `SubscriptionItem.modify(price, proration_behavior="create_prorations")` for both upgrade + downgrade; upgrade path additionally calls `Invoice.create + Invoice.pay` for an immediate prorated charge; downgrade lets the credit ride to the next monthly invoice. `_resolve_plan_change_price_id` maps `starter→PRIVATE_PRICE_ID`, `pro→GLOBAL_PRICE_ID`, rejects `premium` with 422 (no Stripe price configured yet). `brand.scheme_type` is synced post-Stripe-success. Stripe call wrapped in try/except StripeError → 502 with reason. The seed-super bootstrap endpoint at `/api/admin/platform/seed-super` was deleted from `app/main.py` (prod was already bootstrapped + password rotated; the residual security footprint is now closed).
