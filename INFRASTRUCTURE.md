...
* **Plan upgrades / downgrades (`POST /api/billing/plan-change`)** — wired live 2026-05-01 (commit `29b8c09`); downgrade behaviour split from upgrade 2026-05-03 (commit `6d0518d`). The two paths now diverge:
    * **Upgrade:** `stripe.SubscriptionItem.modify(item_id, price=NEW_PRICE_ID, proration_behavior="create_prorations")` followed by `stripe.Invoice.create + stripe.Invoice.pay` so the customer is charged the prorated difference immediately and gets the new tier's benefits in real time. (This is the *plan-change* path; the *quantity-change* path above does NOT call `Invoice.create`.)
    * **Downgrade:** `stripe.SubscriptionItem.modify(item_id, price=NEW_PRICE_ID, proration_behavior="none")` with NO `Invoice.create`. **Revenue-protection policy 2026-05-03:** no credit is issued for the unused portion of the higher tier — the brand finishes the already-paid cycle at the new (lower) rate without a refund. Customer-side: stamps earned at this brand revert to private scope on the next consumer balance read because `main.py:908-994` joins `brand.scheme_type` live; the `stamp_ledger` has no scope column, so no ledger migration is required.
    * Common to both: `_resolve_plan_change_price_id` maps `starter→STRIPE_PRIVATE_PRICE_ID`, `pro→STRIPE_GLOBAL_PRICE_ID`; `premium` returns 422. `brand.scheme_type` syncs post-Stripe-success (best-effort). Pre-flight: 400 if no Stripe subscription/customer, 402 if subscription not ACTIVE. Stripe call wrapped in `try/except StripeError` → 502 with `exc.user_message`.
    * **Frontend dialog** (`b2b-dashboard/src/components/PlanChangeConfirmationDialog.tsx`) branches on direction: upgrade keeps the 3-line receipt; downgrade replaces the prorated-credit row with founder-locked copy ("*You will instantly leave the LCP+ Global network. Because you are exiting mid-cycle, no prorated credits are issued. Your next invoice will be your standard Private rate.*") and a single next-invoice total line.
...
## 5. App Store & Google Play (D-U-N-S received, enrollment in progress)
...
* **D-U-N-S number:** **requested 2026-04-28**, **received 2026-05-03** via Apple's D-U-N-S lookup tool (the dedicated portal at `developer.apple.com` that accepts a CRN and forwards the lookup to Dun & Bradstreet — typically faster than going to D&B directly). Confirmation email + 9-digit D-U-N-S now on file. Original 5–14 day SLA landed mid-window.
* **Apple ID used to submit the D-U-N-S request:** founder's personal Apple ID. **This is harmless.** The D-U-N-S number was generated for the *company* (A Digital Product Studio Limited), not for the Apple ID — a fresh `dev@localcoffeeperks.com` Apple ID can be created and the D-U-N-S applied to that new account during organization enrollment. The submitting Apple ID is irrelevant to the final tie-up.
* **Enrollment status (as of 2026-05-03):** D-U-N-S gating constraint cleared. Enrollment has officially begun on both stores in parallel — see plan below for the steps in flight. Update the bullets as each step completes (paid / awaiting Apple call / awaiting Google ID verification / live).
* **Costs:**
    * Apple Developer Program — **£79/year**, recurring.
    * Google Play Console — **$25 one-time** registration fee.
* **Enrollment plan (now executing):**
...
* **Trap: don't pay either fee before the D-U-N-S record is verified end-to-end.** Both portals will let you start the form but will reject the final submission, and Apple has been known to flag mismatched legal names as suspicious — verify the D-U-N-S record's legal name spells "A Digital Product Studio Limited" exactly before paying. (D-U-N-S is now received but the *legal-name match* on the D&B side is the next thing to confirm before clicking Pay.)
...
* **2026-05-03 (later)** — Section 5 (App Store & Google Play) — D-U-N-S received (requested 2026-04-28; landed 2026-05-03, mid-window of the 5–14 day SLA). Section header retitled "D-U-N-S in flight" → "D-U-N-S received, enrollment in progress". New "Enrollment status" bullet replaces the "Both stores blocked" bullet — gating constraint cleared, both stores running in parallel against the existing 3-step plan (now retitled "Enrollment plan (now executing)"). Trap-line tightened to flag the legal-name match on D&B as the next thing to verify before paying either fee.
* **2026-05-03 (later)** — Section 2 (Stripe Billing) — downgrade plan-changes now use `proration_behavior="none"` (commit `6d0518d`). Revenue-protection policy: no credit is issued for the unused portion of the higher tier; the brand finishes the paid cycle at the new lower rate without a refund. Frontend `PlanChangeConfirmationDialog` branches on direction — drops the prorated-credit row and shows founder-locked copy ("You will instantly leave the LCP+ Global network…") on the downgrade path; upgrade path keeps the existing 3-line receipt + immediate `Invoice.pay`. Customer-side stamps revert to private scope on the next consumer balance read because `main.py:908-994` joins `brand.scheme_type` live (the `stamp_ledger` has no scope column — no ledger migration is needed). Same-day commit `860fc3d` also fixed `BillingView.tsx` which was hardcoding `currentPlan="starter"` regardless of `brand.schemeType`, so a Global brand saw Private as Current and Upgrade-on-LCP+ — an instance of the existing "Tier MUST be threaded from `brand.scheme_type`" rule applied to the in-page display rather than the API call site. Same-day commit `32dbb15` (admin-dashboard) fixed the Super Admin Cafes pill to AND `cafe.billing_status` with `brand.subscription_status` so `incomplete` brands no longer show "Active" while the b2b dashboard / POS gate correctly showed Paused / 402.
