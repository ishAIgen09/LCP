...
def send_tier_upgrade_notification(
    ...
    """Notify a consumer that a brand they've collected stamps at just
    upgraded Private → Global (joined the LCP+ Global Network).

    Sent on the upgrade path of POST /api/billing/plan-change for distinct
    users with a positive net stamp balance at any cafe of the brand at
    the moment of the tier flip. Treated as a transactional system
    notification — not marketing. Idempotency is enforced upstream by
    the `brand.scheme_type != new_scheme` guard in `request_plan_change`,
    so a re-clicked plan change won't double-send.

    The "supercharged" framing is accurate: the consumer balance API
    (`main.py:908-994`) joins `brand.scheme_type` live, so existing
    stamps at this brand instantly count network-wide on the next read
    — no ledger migration is needed."""
    # HTML-escape the brand name everywhere it lands inside HTML
    # attributes / text so a brand named e.g. "Tom & Jerry's" doesn't
    # break the markup. The plain-text fallback is generated by
    # _strip_html on the wrapped HTML and is fine without escaping.
    safe_brand = (
        brand_name.replace("&", "&amp;")
        .replace("<", "&lt;")
        .replace(">", "&gt;")
    )
    subject = f"Great news! Your stamps at {brand_name} just got supercharged ☕️"
    body_html = f"""\
<h1 style="font-size:20px;font-weight:600;line-height:1.3;margin:0 0 12px 0;color:{TEXT_LIGHT};">
  {safe_brand} just joined the LCP+ Global Network
</h1>
<p style="font-size:14px;line-height:1.6;margin:0 0 14px 0;color:{TEXT_LIGHT};">
  Hi there,
</p>
<p style="font-size:14px;line-height:1.6;margin:0 0 14px 0;color:{TEXT_LIGHT};">
  Great news! <strong>{safe_brand}</strong> has just joined the
  <strong style="color:{MINT};">LCP+ Global Network</strong>.
</p>
<p style="font-size:14px;line-height:1.6;margin:0 0 14px 0;color:{TEXT_LIGHT};">
  <strong>What this means for you:</strong> we have automatically
  supercharged your existing stamps from {safe_brand} into LCP+ Global
  stamps! You can now earn <em>and</em> redeem stamps across the entire
  LCP+ network. You don't need to do a thing — just scan your app as
  usual on your next visit.
</p>
<p style="font-size:14px;line-height:1.6;margin:0 0 0 0;color:{TEXT_LIGHT};">
  Enjoy your coffee!
</p>
"""
    return send_email(to_email, subject, _wrap(subject, body_html))
